...
- [ ] UIに触れた場合、E2E（`npm run test:e2e`）が通る（axe-core監査・CLS測定を含む。`e2e/accessibility.spec.mjs`）
...
- [ ] 横スクロールする領域（表・コード片）を足した場合、キーボードでも動かせる（`tabindex="0"`または折り返し。WCAG 2.1 SC 2.1.1）
