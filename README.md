...
- **地点検索**:
  - 都市名・郵便番号から地点を検索（郵便番号はzipcloudで住所へ変換。「蒲郡」のような短い地名は接尾辞を補って再検索）
- **現在地**:
  - GPSで現在地の予報を表示。
  - 座標は約1kmに丸め、保存もURLへの反映もしない設計
- **地点の記憶**:
  - 最後に表示した地点（現在地を除く）をブラウザ内にのみ保存し、次回のアクセス時に自動表示
- **予報の共有**:
  - 表示中の地点の予報をURLで共有（座標は約1km精度）。
  - 任意で日付・プランナーの時間帯（`date`・`from`・`to`）を含められ、開いた側は最新の予報で同じ日時の計画を再現できる
- **イベント固定リンク**:
  - `?event=イベント名`のURLで、登録済みイベントの開催地の予報とプランナー設定まで自動で進む（告知ページへの掲載用）
- **前回との差分表示**:
  - 時間をあけた再訪で日別判定が変わっていれば、差分バナーで悪化・改善を知らせる（この端末内の保存のみ使用）
- **イベント予報**:
  - あらかじめ定義したイベント（`public/events.json`）をリストから選ぶと、郵便番号で引いた開催地の予報を表示。
  - 開催日が予報範囲内ならその日のタブへ自動で切り替え、開催時間は活動プランナーへ設定（[定義の書き方](docs/events.md)）
- **降水確率**:
  - 時間別予報に降水確率を表示（Open-Meteo標準予報APIから補完）
- **実測WBGTツール**:
  - WBGT計（暑さ指数計）の実測値から着ぐるみ判定を確認できる簡易ツール（トップページの「実測WBGT」タブ。判定の根拠と対応表は[説明ページ](https://fursuit-weather.223n.tech/about)に記載）
- **会場表示モード**:
  - イベント会場のモニターに予報を常時掲示する自動表示ページ。
  - 4枚のスライドを自動で切り替えながら予報を定期更新（[使い方](docs/display.md)）
- **風・雷・急な暑さの注意**:
  - 時間別予報に風速を表示し、強風（10m/s以上）・雷を含む天気・急な暑さ（直近7日の平均最高気温を5℃以上上回り、かつ最高気温25℃以上の日）は注意事項欄で知らせる
- **応急対応ページ**:
  - 熱中症が疑われるときの手順（意識確認・着ぐるみの脱がせ方・冷却）をまとめた[もしものときページ](https://fursuit-weather.223n.tech/emergency)。
  - 会場表示モードにも要点のスライドが加わる（判定が厳重警戒以上の間は自動表示）
- **環境省アラート突合**:
  - 表示地点の最寄りの都道府県に熱中症警戒アラート・熱中症特別警戒アラートが発表されているときは、環境省の公式発表として画面へ表示（`GET /api/alert`。自前推定の「基準相当」表示とは別）
- **黄砂・PM2.5の注意**:
  - 大気質データから黄砂・PM2.5の濃度が高い時間帯を注意事項欄で知らせる（毛足の長いファーへの付着・洗浄の目安）
- **着用タイマー・見守り・当日ボード**:
  - 連続活動時間のカウントダウンタイマー（休憩目安と応急対応への導線つき）
  - 10分ごとの自動更新で判定の悪化を知らせる見守りモード
  - 複数の着用者を1台の端末で見守る当日ボード
- **コンディションチェック・暑熱順化・ふりかえり**:
  - 着用前の体調チェック、暑さに慣れるまでの注意の啓発、活動後のふりかえり記録（記録はすべてこの端末内のみに保存）
- **静電気指数・カレンダー・バッジ**:
  - 乾燥期の静電気の起きやすさの目安、登録イベントのカレンダー取り込み（`GET /api/events.ics`）、告知ページへ貼れる当日判定のSVGバッジ（`GET /api/badge.svg`）。
  - SNS共有時のOGP画像も判定入りで動的生成
...
気象APIに接続できない環境では <http://localhost:8787/?demo=1> でデモデータの表示を確認できます。
...
[公開サイトの説明ページ](https://fursuit-weather.223n.tech/about)にもあります。
