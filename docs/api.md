...
この定義の場所は、APIカタログ（RFC 9727）の
`/.well-known/api-catalog`からも機械的にたどれます。

要素名の区切りは、並列のフィールドを「・」、いずれか1つを取る値を
「／」で表記しています。

`/api/*`のほかに、イベント定義`/events.json`も公開しています。
こちらは静的アセットで、クライアントアプリが直接取得しています。
構造は[イベント予報の定義](events.md)を参照してください。

## GET /api/forecast

| パラメータ | 必須 | 説明 |
|-----------|------|--------------------------------|
| `lat` | 必須 | 緯度（`-90`〜`90`。小数2桁へ丸めて処理） |
| `lon` | 必須 | 経度（`-180`〜`180`。小数2桁へ丸めて処理） |
...
座標は範囲を検証したうえで**小数2桁（約1km四方）へ丸めて**から処理します。
上流URLはエッジキャッシュのキーになるため、任意精度の座標を素通しすると、
ほぼ同じ地点の問い合わせがユニークなキーへ際限なく分かれて上流を直接叩けて
しまうためです（`/api/badge.svg`が地点を限定しているのと同じ理由）。
画面側はプライバシー保護のため元から小数2桁しか送らないため、
サービスの表示は変わりません。この丸めは`/api/alert`にも同じく適用されます。

`demo=1`のときは`lat`・`lon`・`days`を検証せず無視し、常に当日からの
3日分（真夏の晴天日・雨天日・曇天日）の決定的なデモデータを返します。
日数は画面の日付タブ（今日・明日・明後日）と同じ3日にそろえてあります。
...
| パラメータ | 必須 | 説明 |
|-----------|------|------|
| `event` | 任意 | `events.json`に登録済みのイベント名。指定するとその1件だけを返します（参加イベントのみ登録したい人向け。未登録の名前は404） |

- `Content-Type: text/calendar; charset=utf-8`と
...
- 低温側の判定（低温注意・低温警戒・低温危険）は、サイト本体と同様に
  青系の配色+雪結晶マークで暑熱側と区別されます（色だけに依存しない）
...
- 発表がないとき・取得に失敗したとき・提供期間外（ファイルなし）・当日5時の
  ...
  異常はWorkerのログで検知します）
- **反映されるのは当日5時発表分のみ**です。前日17時（特別警戒の判定は前日14時）に
  発表される翌日分の予告は反映されません。前日の中止・短縮判断には
  環境省サイトの発表を直接確認してください（17時発表版CSVの突合は
  様式確認待ちの次段課題。`src/api/alert.ts`のコメント参照）
...
活動判定のレベルID（暑熱側・低温側）の一覧を返します。予報レスポンスの
`level`に現れる値を、クライアントが手元の定数とCIなどで機械的に
突き合わせる用途を想定しています。パラメータはありません。

```json
{
  "heat": [
    { "id": "safe", "label": "ほぼ安全", "grade": 0, "activityMinutes": 45 },
    { "id": "caution", "label": "注意", "grade": 1, "activityMinutes": 30 }
  ],
  "cold": [
    { "id": "optimal", "label": "快適", "grade": 0, "activityMinutes": 45 }
  ]
}
```
...
- `heat`・`cold`とも深刻度の低い順に並びます
- 値はサービスの判定表（`src/constants/`）そのものから組み立てるため、
  判定表を変えるとこの応答も同時に変わります
- レスポンスにはブラウザキャッシュ（10分）が付きます
