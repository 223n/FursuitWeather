...
| `staticElectricity` | object | 静電気の起きやすさ（`level`: `low`／`medium`／`high`、`label`: 低／中／高、`advice`: 「高」の日の対策の一言・それ以外は`null`）。日中の最も乾いた時間帯の湿度・気温で判定し、湿度25%未満で「高」、湿度40%未満かつ気温20℃未満で「中」（帯電対策の一般的な目安に基づく生活指数） |
