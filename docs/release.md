...
- アクセシビリティ（axe-core監査・CLS測定）はCIのE2Eジョブが
  5ページ×ライト／ダーク×PC／スマホで実行します。リリース前に手で測る
  手順はありません（詳細は[アクセシビリティ設計](accessibility.md)の検証方法）。
  以前は手動確認としていたため、応急対応ページの119番ボタンのコントラスト比が
  1.70:1へ落ちた状態がv1.9.0〜v1.13.0の7リリースを通過しました
