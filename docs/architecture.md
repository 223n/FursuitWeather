...
「保存しません」はサービスがデータを持たないという意味で、
運用ログ（`wrangler.jsonc`の`observability`によるWorkers Logs）は別枠です。
**ログは2層あり、載る値も出る条件も違います。**

#### invocation logs（Cloudflareがリクエストごとに自動で出す）

Workers Logsはリクエストごとに1件のinvocation logを必ず出します。
Fetchハンドラーのメッセージは`<メソッド> <URL>`で、**クエリ文字列を含みます**。
`console.error`が1行も出ない正常なリクエストでも
`GET https://…/api/forecast?lat=…&lon=…`が残ります。

`wrangler.jsonc`は`"observability": { "enabled": true }`だけを指定しており、
`logs.invocation_logs`は既定のtrueです。止めるには次を足します。
リクエスト単位の切り分け材料（どのURLで落ちたか）を失うため、有効のままにしています。

```jsonc
"observability": { "enabled": true, "logs": { "invocation_logs": false } }
```

画面から届くリクエストの座標は常に小数2桁のため、この経路に残るのも約1kmです。
ただし`/api/*`はCORSが`*`で誰でも直接呼べるため、
任意精度の座標を送られればその値がそのまま残ります。

#### console.error（障害時だけ）

エラー・タイムアウト時に限り、切り分け用の値が出ます。

| 値 | 出る場面 | 精度 |
|----|---------|------|
| 座標 | 上流エラー・予期しない例外・OGP取得失敗 | 小数2桁（約1km）へ丸めてから出す |
| 郵便番号 | 郵便番号で検索したときの上流失敗全般（zipcloudの住所変換の失敗、郵便番号のままの直接検索の失敗・時間切れ、`/api/geocode`の502・予期しない例外） | 約1kmより細かいことがある |
| 検索語（地名） | 地点検索の上流失敗全般（通信失敗・タイムアウト・非2xx・応答の読み取り／解析の失敗・502への変換・予期しない例外） | 利用者の入力そのもの |

座標の丸めは`logSafeSearch`（`src/api/http.ts`）が持ちます。
`url.search`をそのまま出すと`parseLatLonParams`の丸めは戻り値にしか効かず、
丸める前の座標がログへ残るためです。

障害の切り分けに必要な情報のため残していますが、
**郵便番号と検索語は画面が約束する約1kmより細かい情報になりえます**。
ログの保持期間はCloudflare側の設定に従います。
...
計測タグは**配信する5ページすべて**（`index.html`・`about.html`・`emergency.html`・
`404.html`・`display.html`）へ同じトークンで置きます。会場表示モード
（`display.html`）は掲示したまま置かれるため、1回の掲示が1PVとして数えられ、
滞在時間が実際の閲覧者数と対応しません。数字を読むときは他ページと
同じ尺度で比べないでください。

`test/headers.test.ts`が次の3点を検証します。

- HTMLの`<script src>`とCSPの許可元が一致すること（ずれると計測タグが黙って
  ブロックされます）。srcの引用符はシングル・ダブルの両方を拾います。
  ダブルだけを見ていると、Cloudflareのダッシュボードが配る
  シングルクォートのタグを貼ったときにそのページが検査対象から静かに外れます
- ワイルドカードや任意ホストが紛れ込んでいないこと
- 計測タグのトークンが5ページで同一であること（1ページだけ直し忘れると
  計測が2つのサイトへ分かれ、画面にもCIにも出ないまま数字がずれます）

トークンを差し替えるときは5ページすべてを同時に直します（`nonce`はWorkerが
リクエストごとに付けるため、タグ側に書く必要はありません）。
...
### workers.devを有効にしておくことの代償

`workers_dev: true`は障害時の切り分け（「同じコード・同じ上流URLで環境だけが違う」
比較）のために意図的に有効化しています。上の525の節がこの手段に依存しているため、
無効化はしません。

代償として、**223n.techゾーン側に置いた対策はworkers.dev経由で迂回されます**。
将来WAF・レート制限・アクセス制限を入れる場合は、ゾーン設定だけに頼らず
Worker側（`src/`）にも同じ制限を実装してください。上流の無料枠を守る歯止め
（座標の小数2桁への丸め・`/api/badge.svg`の地点限定）をコード側に置いているのは
この理由でもあります。
