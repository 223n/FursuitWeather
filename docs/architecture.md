...
**正常時は何も記録しません**が、エラー・タイムアウト時に限り次が`console.error`へ出ます。

| 値 | 出る場面 | 精度 |
|----|---------|------|
| 座標 | 上流エラー・予期しない例外・OGP取得失敗 | 小数2桁（約1km） |
| 郵便番号 | zipcloudでの住所変換に失敗したとき | 約1kmより細かいことがある |
| 検索語（地名） | 接尾辞補完・郵便番号検索が時間切れになったとき | 利用者の入力そのもの |

障害の切り分けに必要な情報のため残していますが、
**郵便番号は画面が約束する約1kmより細かい情報になりえます**。
