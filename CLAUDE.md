...
- **ページのMarkdown版**（`public/index.md`・`about.md`・`emergency.md`・`display.md`）: `Accept: text/markdown`を送るクライアント（AIエージェントなど）へ、同じURLでMarkdown版を返す（`src/markdown.ts`。ブラウザには従来どおりHTML）。手書きのため**HTMLの節・判定の凡例・注意事項を変えたらMarkdown版も直す**。見出しの増減・凡例の文言・`/about#…`リンクの飛び先は`test/markdownPages.test.ts`が検証する
...
- **`serviceWorkers: 'block'`を外さない**: SWが介在すると`page.route`のモックが素通しされ、2回目以降の遷移でキャッシュが返る。変更が効いていないのに効いて見えるため、計測を誤る。その代わりSW自体（`public/sw.js`）の挙動はE2Eの検証対象外になる。自動検証は`test/sw.test.ts`（sw.jsを読み込み、self・caches・fetchを差し替えてfetchイベントの応答を確かめる）と`test/csp.test.ts`のSHELL_URLS一致。登録・キャッシュの実挙動は実ブラウザでしか確かめられないため、sw.jsを変えたときは手動でも確認する（アプリ内のブラウザのペインではSWを登録できないため、システムのChromeでPlaywrightの`serviceWorkers: 'allow'`と`context.setOffline(true)`を使う）
...
- **PRには必ずラベルを付け、`223n`をアサインする**。ラベルは
  `バグ`・`機能追加`・`ドキュメント`・`リファクタリング`・`アクセシビリティ`・
  `セキュリティ`・`リリース`・`依存関係`・`GitHub Actions`・`JavaScript`から
  該当するものを全部（複数可）。
  変更の種類でPRを絞り込む手段はラベルしかなく、後から遡って付けるとPRの本数だけ
  手作業になる（実際に103本へ後追いで付け直した）。DependabotのPRのラベルと
  アサインは`.github/dependabot.yml`の`labels`・`assignees`が受け持つ
  （`labels`を指定しないと英語名の既定ラベルが作り直される）
